...
import { Sun, Moon, Menu, X, ArrowRight, Mail, Phone, MapPin, Download, ExternalLink, Palette, Monitor, FileText, Smartphone, TrendingUp, Package, Github, Linkedin, Instagram } from 'lucide-react';

// 3D Components
const FloatingGeometry = ({ position, color, geometry = "box" }: { position: [number, number, number]; color: string; geometry?: string }) => {
  ...
  return (
    <Float speed={1.4} rotationIntensity={1} floatIntensity={2}>
      <mesh ref={meshRef} position={position}>
        ...
        {geometry === "cone" && <coneGeometry args={[0.6, 1.2, 8]} />}
...
const Scene3D = ({ isDark }: { isDark: boolean }) => {
  return (
    <Canvas camera={{ position: [0, 0, 5], fov: 60 }}>
      ...
      <FloatingGeometry position={[-2, 1, 0]} color={isDark ? "#3b82f6" : "#2563eb"} geometry="box" />
      <FloatingGeometry position={[2, -1, 0]} color={isDark ? "#8b5cf6" : "#7c3aed"} geometry="sphere" />
      <FloatingGeometry position={[0, 1, -1]} color={isDark ? "#06b6d4" : "#0891b2"} geometry="cone" />
  ...
  return (
    <div className={`${isDark ? 'dark' : ''} font-inter`}>
      <div className="min-h-screen bg-white dark:bg-[#0a0a0a] text-[#0f172a] dark:text-white transition-colors duration-300">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex justify-between items-center h-16">
              ...
              <div className="flex items-center space-x-4">
                <motion.button
                  onClick={toggleTheme}
                  className="p-2 rounded-lg bg-[#f8fafc] dark:bg-[#1a1a1a] hover:bg-gray-200 dark:hover:bg-[#2a2a2a] transition-colors"
                ...
                <button
                  ...
                  className="md:hidden p-2 rounded-lg bg-[#f8fafc] dark:bg-[#1a1a1a]"
                >
                ...
                <div className="px-4 py-6 space-y-4">
                  {['Work', 'Services', 'About', 'Contact'].map((item) => (
                    <button
                      key={item}
                      onClick={() => scrollToSection(item.toLowerCase())}
                      className="block w-full text-left text-[#64748b] dark:text-[#a3a3a3] hover:text-blue-600 dark:hover:text-blue-400 transition-colors duration-200"
                    >
                      {item}
                    </button>
                  ))}
        ...
        {/* Work Section */}
        <section id="work" className="py-20 bg-[#f8fafc] dark:bg-[#1a1a1a]">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <motion.div
              initial="initial"
              whileInView="animate"
            ...
            >
              {projects.map((project) => (
                <motion.div
                  key={project.id}
                  variants={fadeInUp}
                  className="group bg-white dark:bg-[#0a0a0a] rounded-xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300"
                  whileHover={{ y: -10 }}
                >
                  ...
                  <div className="p-6">
                    ...
                    <p className="text-[#64748b] dark:text-[#a3a3a3] mb-4">{project.description}</p>
        ...
        {/* About Section */}
        <section id="about" className="py-20 bg-[#f8fafc] dark:bg-[#1a1a1a]">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                  ...
                  <div className="flex gap-4">
                    {[
                      ...
                      <motion.a
                        key={index}
                        href={social.href}
                        className="w-10 h-10 bg-[#1a1a1a] hover:bg-blue-600 rounded-lg flex items-center justify-center transition-colors duration-200"
                        whileHover={{ scale: 1.1 }}
                        whileTap={{ scale: 0.9 }}
                      >
